    ...
    <div className="page">
      ...
      <div className="toolbar" style={{ flexWrap: "wrap", gap: "0.75rem", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: "1rem", marginLeft: "auto", fontSize: "0.85rem", alignItems: "center" }}>
          ...
          <label style={{ display: "inline-flex", alignItems: "center", gap: "0.3rem", cursor: "pointer" }}>
            <input type="checkbox" checked={showPasswordCol} onChange={(e) => setShowPasswordCol(e.target.checked)} />
            Password
          </label>
        </div>
      </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                {showPasswordCol && <th>Password</th>}
                <th>Status</th>
                <th>Files</th>
              </tr>
            </thead>
            <tbody>
              {users.map((user) => (
                <tr key={user.id} className={selected?.id === user.id ? "selected-row" : ""} onClick={() => selectUser(user.id)}>
                  ...
                  {showPasswordCol && (
                    <td>
                      <code style={{ fontSize: "0.75rem", background: "#f1f5f9", padding: "2px 5px", borderRadius: "4px" }}>
                        {user.hashed_password ? `${user.hashed_password.substring(0, 10)}…` : "—"}
                      </code>
                      {user.hashed_password && (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            navigator.clipboard.writeText(user.hashed_password || "");
                            setMessage(`Password hash for ${user.email} copied!`);
                          }}
        ...
        {selected ? (
          <section className="panel detail-panel">
            ...
            <dl className="detail-list">
              ...
              <div>
                <dt>Password Credential</dt>
                <dd style={{ wordBreak: "break-all", fontSize: "0.85rem" }}>
                  <code>{selected.hashed_password || "N/A"}</code>
                  {selected.hashed_password && (
                    <button
                      ...
                      onClick={() => {
                        navigator.clipboard.writeText(selected.hashed_password || "");
                        setMessage(`Copied password credential for ${selected.email}`);
                      }}
                    >
                      Copy
                    </button>
