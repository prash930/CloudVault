...
def get_connection(
    db: Session,
) -> Optional[GoogleDriveConnection]:
    return (
        db.query(GoogleDriveConnection)
        .filter(GoogleDriveConnection.id == 1)
        .first()
    )
...
def verify_oauth_state(
    ...
    if db is not None:
        ...
        if (
            ...
            raise ValueError(
                "Admin user in OAuth state is invalid or inactive"
            )
...
def _initialize_drive_folders(
    ...
    root_folder_id = _create_folder(
        service,
        "root",
        "CloudBox",
    )

    users_folder_id = _create_folder(
        service,
        root_folder_id,
        "users",
    )
...
def exchange_code_and_connect(
    ...
    root_folder_id, users_folder_id = (
        _initialize_drive_folders(refresh_token)
    )
